/*
* This file defines the required adjustments for the EBS CSI driver.
* The specifications that are defined in this file are used in the cluster_services.main.tf file.
*
* Details:
* 1. EBS Encryption enables encryption of volumes at rest. It also allows to encrypt the snapshots.
* 2. You can run the Amazon EBS CSI controller on Fargate, but you can't mount volumes to Fargate pods.
* 3. Alpha features of the Amazon EBS CSI driver aren't supported on Amazon EKS clusters.
* 4. This modules installs the EBS CSI driver as a self-managed addon.
*
* Assumptions:
* - The IAM Roles for Service Accounts (IRSA) is enabled by default.

* Reference:
* - [How to configure the IAM Role for the <ebs-csi-controller-sa> Service Account?](https://docs.aws.amazon.com/eks/latest/userguide/csi-iam-role.html);
...
locals {
  ebs_csi_driver = {
    name                = "AmazonEKS_EBS_CSI"
    namespace           = "kube-system"
    k8s_service_account = "ebs-csi-controller-sa"
    k8s_oidc_issuer_url = var.kubernetes_oidc_issuer_url
...
resource "aws_iam_policy" "ebs_controller_encryption_policy" {
  name_prefix = local.ebs_csi_driver.name
...
module "ebs_controller_role" {
  ...
  role_name_prefix              = local.ebs_csi_driver.name
  provider_url                  = local.ebs_csi_driver.k8s_oidc_issuer_url
  role_policy_arns              = concat([aws_iam_policy.ebs_controller_encryption_policy.arn], ["arn:aws:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicy"])
  oidc_fully_qualified_subjects = ["system:serviceaccount:${local.ebs_csi_driver.namespace}:${local.ebs_csi_driver.k8s_service_account}"]
  tags                          = local.tags
}

resource "kubernetes_service_account_v1" "ebs_csi_driver" {
  metadata {
    name      = local.ebs_csi_driver.k8s_service_account
