locals {
  cluster_metrics_server_default_values = <<VALUES
spec:
  resources:
    requests:
      cpu: 100m
...
module "kube_metrics_server" {
  ...
  cluster_metrics_server_helm_chart_repository          = try(coalesce(var.cluster_metrics_server.helm_chart_repository, "oci://registry.jetbrains.team/p/helm/library"), "oci://registry.jetbrains.team/p/helm/library")
  ...
  cluster_metrics_server_default_values_dot_yaml        = try(coalesce(var.cluster_metrics_server.helm_chart_values, local.cluster_metrics_server_default_values), local.cluster_metrics_server_default_values)
